...
esp_err_t lora_at_config_create(lora_at_config_t **config) {
  ...
  ERROR_CHECK_IGNORE_NOT_FOUND(nvs_get_blob(out_handle, "minFrequency", &result->min_freq, &float_size));
  ERROR_CHECK_IGNORE_NOT_FOUND(nvs_get_blob(out_handle, "maxFrequency", &result->max_freq, &float_size));
